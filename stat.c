...
void bernoulli_gen(int v[], int n, double probability) {
    fill_with_randoms(v, n, 0, RAND_MAX - 1);
    for(int i = 0; i < n; i++) {
        v[i] = (v[i] < (probability * RAND_MAX)) ? 1 : 0;
    }
}
...
void fill(double v[], int p, int q, int val) {
    for(int i = p; i < q; i++) {
        v[i] = val;
    }
}
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
    ...
    for(int i = x_start; i < x_start + n; i++) {
        ...
        int range = v[i] / y_scale;
        if(v[i] - range * y_scale > 0.5 * y_scale) range++;
...
void monty_hall(int n, int *p_switch_wins) {
    *p_switch_wins = 0;
    for(int i = 0; i < n; ++i) {
        int price = m_rand(1, 3);
        int firstChoice = m_rand(1, 3);

        if(price != firstChoice) {
            (*p_switch_wins)++;
        }
    }
}
